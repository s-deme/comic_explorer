...
| TypeScript/frontend | PASS | 2026-08-12、25 files / 253 tests PASS、FAIL 0。root登録画面を表示しない起動、PC配下のdrive表示とsidebar選択、drive切替、現在folder absolute pathのtree header、navigation・drive往復・検索pane・一時非表示をまたぐbranch展開保持、明示的な全折りたたみ、inactive driveの保持済みfolderからのdrive/path移動を含む。`\\?\` / `\\?\UNC\`を除去したaddress表示、引用符付きabsolute path移動、一覧形式の提示順・名称、形式別固定thumbnail寸法、設定のatomic保存、profile移行、command、検索、viewer等の既存回帰とTypeScript typecheckもexit 0。 |
| Python | PASS | 2026-08-12、49 tests PASS、FAIL 0。tree headerと独立scroll領域、設定dialog、catalog固定幅card、画像とファイル名の分離、狭幅時にもthumbnailを上書きしないstyle contract、現行status/verification間の5値consistencyもPASS。 |
