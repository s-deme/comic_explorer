...
| TypeScript/frontend | PASS | 2026-08-12、25 files / 249 tests PASS、FAIL 0。root登録画面を表示しない起動、PC配下のdrive表示とsidebar選択、drive切替、`\\?\` / `\\?\UNC\`を除去したaddress表示、Windows Explorerからコピーした引用符付き絶対pathのaddress移動、separator・case正規化、driveのpath segment境界と親directory脱出拒否を含む。一覧形式の提示順・名称、形式別固定thumbnail寸法、ウィンドウ幅による列数だけの変更、ファイル名専用領域、設定のatomic保存とprofile v1からv2への移行を検証した。設定の5カテゴリnavigation、横断検索、command表、予約・重複key拒否、catalog/viewer command実行、mouse入力、対応書庫表示、検索、viewer等の既存回帰とTypeScript typecheckもexit 0。 |
| Python | PASS | 2026-08-12、49 tests PASS、FAIL 0。設定dialogの固定header/body/action、カテゴリnavigation、6列command表、説明付きrow、狭幅1列化に加え、catalogの固定幅card、画像とファイル名の分離、狭幅時にもthumbnailを上書きしないstyle contract、現行status/verification間の5値consistencyもPASS。 |
