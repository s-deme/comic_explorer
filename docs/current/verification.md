...
| 領域 | 状態 | 実測内容または境界 |
|---|---|---|
| Rust canonical | PASS | 2026-08-11、lib 148件 + shutdown process 1件、FAIL 0。検索のscope・種別・size・更新日時・固定場所の条件適用を含む。`cargo fmt --check`と`cargo check --locked`もexit 0。 |
...
| TypeScript/frontend | PASS | 2026-08-11、24 files / 201 tests PASS、FAIL 0。対応書庫の項目別形式表示、検索条件のrequest変換・検索結果保持、一般ヘルプとバージョン情報の分離、診断のread-only説明と実行中indicator、catalogの利用可能幅に応じたcard列数・virtual行の縮退、サムネイル系cardの種別非表示と画像・長いファイル名のlayout分離、TypeScript typecheckもexit 0。 |
| Python | PASS | 2026-08-11、41 tests PASS、FAIL 0。menu/addressのcompact寸法、tree文字色、固定page幅を持たないcatalog縮退、検索条件・診断indicatorのstyle contract、現行status/verification間の5値consistencyもPASS。 |
...
Python 41件とfrontend 198件を実行して全件PASSし、typecheckを実行、source integrityを確認した。
