...
順序付き処理手順へ分けた。現在使用中のセットと手順数を表示し、dialogは少ない手順でcontent高へ縮み、
多数手順だけをpanel内scrollへ収める。container queryで狭幅を1列化し、buttonの横overflowや切り落としを
防ぐ。

| Gate | 結果 | 2026-08-28の実測 |
|---|---|---|
| Focused FilterDialog / Viewer / UI style | PASS | FilterDialog 4件、Viewer 61件、Python UI style 31件、FAIL 0。セット・action・順序付き手順の分離と狭幅CSS契約を確認。 |
| Windows tests | PASS | Python 75件、frontend 44 files / 599件、FAIL 0。 |
| TypeScript typecheck / frontend build | PASS | `run-typecheck-windows.ps1` exit 0。85 modulesのproduction buildがexit 0。minify後673.26kBの既存chunk advisoryはFAILへ読み替えない。 |
| 製品直接観測 | NOT RUN | この環境ではローカルbrowser接続を取得できず、release WebView2での通常幅・狭幅・DPI別の目視操作は未測定。 |
...
2026-08-23にGitHub ActionsのNode.js 20廃止へ対応し、全workflowの`actions/checkout`を`v5`、
`actions/setup-node`を`v5`、`actions/setup-python`を`v6`、`actions/upload-artifact`を`v6`へ更新した。
release evidence 8件はNode.js 24 runtime版の採用と旧major不在を含めてPASSし、3 workflowのYAML parse、
Python 64件、frontend 41 files / 515件、TypeScript typecheckもPASSした。Action実行結果はpush後の
GitHub Actions runで観測する。
...
同日の追加更新で、書庫一覧のfolder・nested archive・画像は1clickで選択し、double clickまたはEnterで開く操作へ統一した。選択画像の明示buttonとCtrl+Cは、Rustがroot-contained archiveとopaque page keyを再検証し、既存上限内でBGRA decodeした画像だけをWindows `CF_DIBV5` clipboardへ渡す。focused frontend/clientは2 files / 32件、Rust clipboard decode 1件、Windows全体回帰はPython 65件、frontend 41 files / 517件、Rust 257件とshutdown process 1件、typecheckをPASSした。最初のfrontend全体runは旧single-click openを期待するApp test 1件だけFAILし、期待をdouble-clickへ更新後に単独および全517件をPASSした。実書庫から他applicationへのpasteはrelease WebView2で未測定とする。
...
## Leeyes P5-B 非破壊画像フィルター

対象はLEY-FILTER-001〜016の16件。Rustが14種のpixel処理、chain検証、named/active set、SQLite schema v12、WIC decodeとbounded PNG生成を担当し、TypeScriptはtyped IPCとReact/WebViewのfilter editorだけを担当する。原本、thumbnail cache、書庫、PDFは変更しない。

| gate | 結果 | 証拠・未測定 |
|---|---|---|
| Focused Rust | PASS | REQ-LEY-P5-002の6件、FAIL 0。14処理の代表pixel、順序差、alpha、crop/margin寸法、parameter拒否、WIC→PNG、SQLite CRUD・単一activeを含む。 |
