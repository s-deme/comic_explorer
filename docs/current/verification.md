...
| TypeScript/frontend | PASS | 2026-08-12、25 files / 238 tests PASS、FAIL 0。Windows Explorerからコピーした引用符付き絶対pathのaddress移動、separator・case正規化、rootのpath segment境界と親directory脱出拒否を含む。設定の5カテゴリnavigation、名前・説明・現在値を対象にする横断検索、group/command/key/mouse/説明のcommand表、予約・重複key拒否、catalog/viewer command実行、9種類の変更可能mouse入力、固定double click、旧設定移行と新既定値との衝突回避、atomic適用・profile入出力を含む。対応書庫表示、検索、viewer、catalog等の既存回帰とTypeScript typecheckもexit 0。 |
