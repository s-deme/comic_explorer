...
toolbarの検索buttonはfolder treeと名前検索専用paneを切り替える。basename maskは「現在の一覧を絞り込む」filter barとしてcatalog上部へ分離し、検索結果には適用しない。検索paneとfilter barはfolder/fileの肯定形selector componentを共有するが、filesystem走査と列挙済みsnapshot評価の実行境界は統合しない。catalogは
