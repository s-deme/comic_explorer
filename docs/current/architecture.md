...
viewer toolbarは作品名と主要なpage・表示枚数・倍率・しおり・window操作を1行のprimary controlとして優先し、低頻度の巻末、pixel寸法、補助zoom、slideshow、見開き調整、読み方向、しおり管理、分離、copy、画像変換・filterを展開式secondary controlへ集約する。primary rowは横scrollを作らず、狭幅では作品名と段階zoomを視覚的に縮退するがaccessible nameとkeyboard commandは維持する。
