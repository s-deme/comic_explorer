...
現在pageはtoolbarではなく、画像表示領域下部のrange slider式page移動barで総page数とともに示す。sliderはRTL方向として先頭pageを右端、末尾pageを左端へ配置し、viewer modelの`go` commandへ接続して任意のpageを表示する。見開き開始位置を1pageずつ戻す/進める、前page、次page、random page、slideshowは同bar右端のgroupに置き、barが全画面でoverlay表示されるときも同じgroupとして表示・非表示を共有する。
全画面中のviewer toolbarとpage移動barはlayout領域を占有せず、toolbarは画面上端、page移動barは画面下端へpointerを移動したときだけ表示し、各controlから画像領域へ離れると再び隠す。
Viewerはmain shell内の画面切替ではなく、label `viewer` の再利用可能なTauri WebviewWindowへ置く。mainはViewerを開くためにcatalog、選択、scroll、thumbnail stateをunmountまたはclearせず、初回だけhash routeで相対item key・開始mode・開始page keyを渡す。相対keyはViewer windowが既存の`open_comic`境界で再検証してsessionを取得するため、window routeを任意path accessへ使わない。既存Viewer windowには対象をeventで渡して前面化し、同じwindowのsessionだけを切り替える。Viewerが閉じた時はwindowだけを閉じてmainへfocusを戻し、mainはfolder一覧を再列挙しない。page変更はitem/page keyだけをmainへ通知して、設定が有効な場合だけ既存catalogの選択と同期する。Viewer windowは同じsettings、theme、bookmark、読書位置、page先読み、巻末policyをbackendから取得し、疑似的な同一window内の「画像表示を分離」は提供しない。toolbarは閉じるを先頭側に固定し、作品名はnative titleだけに表示する。右側は表示枚数、倍率、ルーペとしおりと補助操作、全画面の4 groupをこの順に区切って並べ、ルーペはしおり保存の直前に置く。見開きの1page調整、random page、前後page、slideshowはpage移動bar右端の操作groupへ集約する。低頻度の巻末、pixel寸法、矩形zoom、読み方向、しおり管理、copy、画像変換・filterは「その他の操作」から開く用途別のoverlay panelへ置く。panelは表示とサイズ、移動と読み方、しおりと共有、画像の4群を可読な名前付きbuttonとinputで示し、「移動と読み方」内には読み方向だけを置く。通常表示でもtoolbarの固定高さ・overflowに依存せず画像のlayoutを押し潰さない。primary rowは横scrollを作らず、狭幅では段階zoomを視覚的に縮退するが、「閉じる」を折返しやoverflowで隠さず、accessible nameとkeyboard commandを維持する。Viewer mountはTauri window adapterからnative titleを`Comic Explorer — <作品名>`へ設定し、作品切替では更新、unmountでは`Comic Explorer`へ戻す。native title API失敗はViewer lifecycleへ波及させない。
...
viewerの回転・反転は原画像、Rust decode、media grant、clipboard、cacheを変更しないsession内の共通表示状態とする。時計回り90度、screen軸の左右・上下反転、resetをO(1)で合成し、同一sessionの全page、見開き、移動先、loupeへ適用する。90度・270度ではnatural寸法の縦横を入れ替えてauto spread判定とfit倍率へ渡し、CSS transformをmain imageとloupe surfaceへ同じ順序で適用する。回転角と左右・上下反転のON/OFFは状態表示と各buttonのpressed状態で示す。固定keyは`]`、`H`、`V`、`0`とし、modifier、編集control、contenteditable、dialogからは発火させない。Viewerの対象切替または終了で状態を破棄し、profile・SQLite・原本へ保存しない。
...
入力編集はoptionsの統合設定だけから行い、「キー設定」「マウス設定」「ジェスチャー設定」を独立して切り替える。キー設定はcatalogとviewerのcommandをgroup、1〜4個のkeyboard binding、説明の表で扱い、キーによるpage内移動も併設する。マウス設定はcatalog click、Viewerの4象限clickと右click、drag panを扱う。ジェスチャー設定はViewer画像stageだけを対象に左右swipe、wheel、右button+wheel、middle/side buttonの割当とwheel不感帯を表示し、double clickは全画面toggle固定として説明する。Rustのshortcut registryは既知command、正規key、順序、件数、command内重複、command間と予約済みapp操作の衝突を検証し、SQLiteへ配列としてatomicに保存する。strict profile v22も配列を必須とし、v1〜v21と既存SQLiteの単一文字列は1要素配列へ移行する。TypeScriptはkeyboard eventを正規形へ変換してRust検証済み配列を即時照合し、draft UIとdispatchだけを担う。mouseはviewer stageで直接観測できる
