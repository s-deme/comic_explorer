...
| REQ-LEY-P3-003 | LEY-CATALOG-007 | catalog maskへ、folderを含む・fileを含む、最小/最大size KiB、更新日の開始/終了をAND条件として追加し、少なくともfolder/fileの一方を必須とする。size条件はmetadataのない項目を除外し、更新日はlocal dateの開始以上・終了日の翌日未満として扱い、開始超過・終了超過、負数、非有限値、不正日付をRust境界で`INVALID_REQUEST`として拒否する。名前式と詳細条件は1回のRust batch評価で短絡し、invalid・cancel・stale応答では最後のvalid表示と選択を保持する。条件一式を1〜64文字の名前でapp-local SQLiteへ最大32件保存し、同名保存は内容をatomicに置換する。保存済み条件を選ぶと全draftを復元し、明示適用するまで表示を変えず、削除は対象名を確認してから行う。保存順は更新が新しい順で、再起動後も復元するが、library原本・catalog snapshot・検索場所・profile exportへは書き込まない。名前、式、種別、size、日付はstrictに検証し、絶対pathや任意codeを保存・実行しない。Leeyesの既定値・保存上限・日付境界は未確認のため、全件相当の安全な既定値、32件上限、local calendar dayの半開区間を明示的な現代化として採用する。10,000項目の複合評価を2秒未満で測定し、release SQLite再open・WebView2大量IPCを未観測なら未測定と記録する。 |
| REQ-LEY-P3-004 | LEY-SEARCH-001, LEY-SEARCH-002 | 名前検索のsourceを現在libraryだけに固定せず、Windows標準folder pickerで利用者が明示選択した読み取り可能なfolderをsession中最大8件まで追加し、選択sourceを1回の検索で順番に横断する。picker cancelは正常終了で現在sourceを保持し、任意のfrontend文字列は検索sourceとして受理せず、Rustがcanonicalizeしてsession allowlistへ登録したpathと現在library rootだけを許可する。sourceはcanonical pathのcase-insensitive一致で重複排除し、0件を拒否する。複数source時は現在folder固定条件を無効化して各source rootから同じquery・subfolder・kind・size・date条件を適用し、重なるsourceの同一canonical itemは先に指定したsourceの結果1件へ統合する。全source合計50,000結果で`RESOURCE_LIMIT`とし、cancel/stale generationを既存境界で破棄する。結果はsource rootとsource内relative pathを分離して表示し、同名でもsourceが違えば区別する。結果を開くと選択したsourceを新しいlibrary rootとしてbackendで再検証・登録して親folderへ移動し、検索前rootへ文字列を注入しない。source追加・削除はsession限定でprofile/SQLiteへ保存せず、library原本、index、cacheを変更しない。Leeyesのsource種別・重複優先順位は未確認のため、明示選択folderをsource、入力順優先のcanonical重複排除を安全な現代化として採用する。2つ以上の実filesystem source、重なり、missing/access、50,000上限、cancelをRust testで検証し、release folder pickerとslow/removable driveを未観測なら未測定と記録する。 |
...
| REQ-LEY-P3-015 | LEY-INPUT-009 | Viewer stageのmodifierなしright mouse clickへ、`none`または既存の安全なViewer commandを1つ割り当てる。既定は`none`として現行の無操作を保ち、paged・vertical scroll・horizontal scrollの全layoutで同じ割当を使う。mouseの右button down/upが同一pointerで4px未満の移動に収まった場合だけpointer upで1回dispatchし、右buttonを押したままのwheelは既存のright-wheel割当を優先してright-click actionを取消する。4px以上の移動、touch、pen、modifier付き入力、pointer cancel、window blur、toolbar・dialog・control、catalog・folder treeのcontext menuでは発火させない。Viewer stageのbrowser context menuは従来どおり抑止するが、catalogとfolder treeのfile operation context menuは固定操作として変更しない。Rustのviewer right-click registryを正本とし、`none`を含む既知Viewer actionだけをprofile保存前・import時に検証してapp-local SQLiteへatomic保存する。strict profile v26でbindingを必須化し、profile v1〜v25と既存DBには`none`を補う。TypeScriptはWebView pointer種別・button・modifier・座標、4px click/drag判定、right-wheel成立の取消、設定draft、Rust検証済みactionの既存Viewer handlerへのdispatchだけを担当し、設定正本、filesystem、任意code、外部app、global hookを持たない。Rust testで既知・未知action、default、profile移行、SQLite再openを、frontend testでsingle dispatch、movement・modifier・touch/pen・cancel保護、right-wheel優先、全layout、catalog context menu非回帰、設定保存を検証する。release WebView2の実right-click順序、touchpad、多ボタンmouse、DPI、input latencyを未観測なら未測定と記録する。 |
