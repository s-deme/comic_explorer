...
| 不足・差分 | 今回の対応 | 残る範囲 |
|---|---|---|
| ページ移動前に画像を確認できない | 番号・画像・ファイル名のプレビュー、確定時だけ移動、失敗時の再試行を追加 | サムネイルを並べる全ページ一覧は対象外 |
| アニメーションWebPを拒否していた | フレーム検証付き表示と先頭フレームの派生画像生成へ接続 | 1024フレームとキャンバスメモリ上限、フィルター適用時は静止画 |
| AVIFの実装と説明が不一致 | 既存の判別・表示経路と環境依存の制限をREADMEへ反映 | Windowsコーデック非依存のサムネイル生成は未対応 |
| 連続縦読み・ネットワーク・色管理 | 利用者指定の画像形式・プレビューを先行 | 縦読みはpaged契約の変更、通信はoffline境界の検討が必要 |
