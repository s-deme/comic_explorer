...
ウィンドウ拡大時は中央一覧の列数と表示行数を増やし、ツリー幅は維持する。縮小時はツリーと一覧に最小幅を設け、縦スプリッターで両ペインをリサイズ可能とする。最小幅を下回るサイズにはウィンドウを縮小できない。上部3領域とステータスバーの高さは固定し、中央領域だけを伸縮する。ツリーと一覧はそれぞれ独立して縦スクロールする。

### 4.2 領域定義

| 領域 | 内容・操作 |
| --- | --- |
| メニューバー | `ファイル`: ルート選択、終了。`表示`: 並べ替え条件・方向。`移動`: 戻る、進む、上へ。`ヘルプ`: キー操作の確認。MVP対象外コマンドは置かない |
...
移す。条件と方向はアプリ再起動後の永続化要否が要件にないため TBD-SF-003 とする。
...
ウィンドウサイズ変更時は上部バーを固定し、残りを表示領域とする。単ページは画像全体が収まる最大倍率、見開きはページ間の間隔を含む最大2ページ全体が収まる共通倍率を用いる。拡大はしないという要件はないため、小画像を表示領域より大きく拡大するかは TBD-SF-004 とする。常にアスペクト比を維持し、画像を変形・切り抜きしない。
...
| OP-006 | SCR-LIB | 一覧表示済み | メニュー、ツールバー | 条件または方向を選択 | 欠落なく再配列し表示を更新 | 現在順を維持しエラー表示 | TBD-SF-003 | REQ-MVP-007 |
...
| OP-011 | VIEW-1/2 | ビューワ表示中 | `1`/`2`、上部ボタン | 単ページ／見開き切替 | 先頭ページ維持で再配置 | 元モードを維持し理由表示 | 要件なし（位置のみ保存） | REQ-MVP-011,012,015 |
...
| 並べ替え条件・方向 | アプリ状態／名前・昇順を初期案 | OP-006 | 一覧表示と次項目決定で同一値を使用 |
| 開いている漫画項目 | ビューワセッション | OP-008/014、終了 | 元親フォルダと復帰コンテキストを保持 |
| 現在ページ | 漫画項目別・永続 | 表示成功 | 相対パスを主、順序を補助として保存 |
| 表示モード | アプリ状態／初期値はTBD-SF-006 | OP-011 | 切替時に先頭ページを維持。永続化はTBD |
...
| ID | 未決事項 | 決定に必要な情報／影響 |
| --- | --- | --- |
| TBD-SF-003 | 並べ替え条件・方向をアプリ再起動後も保持するか | 永続設定範囲。次項目規則にも影響 |
| TBD-SF-004 | 全体フィット時に小さい画像を原寸より拡大するか | 画質、読みやすさ、性能の比較 |
| TBD-SF-006 | 初回の表示モード、および表示モードを再起動後も保持するか | 単ページ／見開きの既定動作と設定範囲 |
| TBD-SF-007 | 最小ウィンドウ寸法、ツリー最小／初期幅、サムネイル寸法 | Windows実機での文字倍率・高DPI評価 |
| TBD-SF-008 | エラー理由の利用者向け分類と文言 | OS／書庫処理から得られるエラー情報とセキュリティ配慮 |
| TBD-SF-009 | `ヘルプ` メニューでキー一覧をアプリ内表示するか、同梱文書を開くか | 配布設計とオフラインヘルプ方針 |

TBD-SF-001、002、005は2026-07-29にMVP要件へ確定反映した。性能基準PC、
