...
environment and Windows Node.js/npm, and must not depend on WSL `/mnt/*`
...
- The PowerShell runners default to the Windows virtual environment
  `.venv-windows`; the Linux/WSL `.venv` is never selected implicitly.
- A PowerShell consistency runner executes the existing producer and validates
  the `depends_on_consistency` JSON result without a Bash or WSL pipeline.
