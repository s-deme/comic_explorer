...
| TC-NFR-006-001 | NFR-MVP-006-AC6 | Performance/Compatibility | P0 | fixture generatorとclean output領域、Windows/WSL/Linuxの各実行環境 | 内部固定SEED `20260728`で生成したmalformed ZIP/image/security corpus | 各環境で内部固定SEEDを用いてfixtureを生成し、manifestのfixtureSeedを確認する。利用可能なplatform path変換interfaceを検出し、interface有り・無しの両分岐を実行する。replace指定なしで既存出力がある場合は生成を拒否（非zero）することを確認し、既存出力の内容hashとmtimeを比較する。`--force`/`-Force`時は許可対象directoryだけ再作成する | Windows、WSL、通常Linux CIで内部固定SEED `20260728`とmanifestのfixtureSeedが一致し、fixtureが同一manifest（fixture種別・相対path・内容hash）となる。path変換は利用可能interface検出時だけ実行され、interface無しでは未変換pathを使う。replace指定なしで既存出力がある場合は拒否（非zero）となりhash/mtimeは不変。`--force`/`-Force`時は許可対象directoryだけ再作成でき、決定的manifest/hashが同一となる（hashまたはmtimeの変更は必須としない）。結果は実測またはBlockedを環境別に記録し、未実行をPASSにしない | 内部SEEDまたはfixtureSeed不一致、manifest/hash不一致、interface無しでのpath変換、replace指定なしの既存出力上書き、拒否時のzero終了、hash/mtime変化、許可外directory再作成、または未実行なのにPASS記録 | 標準 | Blocked | failure oracle: 期待結果のいずれかを満たさない、または環境・分岐・replace条件・証跡（fixtureSeed/manifest/hash/mtime）が欠けた場合はFAIL。通常LinuxでPowerShellが欠落している場合は環境別Blocked、実行済み分岐の失敗はFAIL。Windows実機等の未実行はBlocked |
...
| UI／E2E／Error | 23 |
| Performance／Accessibility／Distribution | 13 |
| **合計** | **73** |

P0は48件、P1は25件である。要件ギャップ解消後の現時点で `Specified` は36件、
`Blocked` は37件である。残る`Blocked`は製品実装、lockfile、Windows実機または
