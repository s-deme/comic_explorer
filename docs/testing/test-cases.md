...
| TC-NFR-006-001 | NFR-MVP-006-AC6 | Performance/Compatibility | P0 | fixture generatorとclean output領域、Windows/Linux CIの各実行環境 | 内部固定SEED `20260728`で生成したmalformed ZIP/image/security corpus | 各環境で内部固定SEEDを用いてfixtureを生成し、manifestのfixtureSeedを確認する。既存出力は明示的な置換指定なしに上書きしない。`--force`/`-Force`時は許可対象directoryだけ再作成する | Windows、通常Linux CIで内部固定SEED `20260728`とmanifestのfixtureSeedが一致し、fixtureが同一manifest（fixture種別・相対path・内容hash）となる。既存出力は置換指定なしで拒否（非zero）となりhash/mtimeは不変。`--force`/`-Force`時は許可対象directoryだけ再作成でき、決定的manifest/hashが同一となる。結果は実測またはBlockedを環境別に記録し、未実行をPASSにしない | 内部SEEDまたはfixtureSeed不一致、manifest/hash不一致、replace指定なしの既存出力上書き、拒否時のzero終了、hash/mtime変化、許可外directory再作成、または未実行なのにPASS記録 | 標準 | Blocked | failure oracle: 期待結果のいずれかを満たさない、または環境・証跡（fixtureSeed/manifest/hash/mtime）が欠けた場合はFAIL。実行済み分岐の失敗はFAIL。Windows実機等の未実行はBlocked |
