...
| TC-UI-007 | PASS | release WebView2で127項目を4条件×昇降順すべて全scroll収集し独立oracleと一致、欠落重複0、null欠損末尾、選択/focus維持、種類降順の再起動復元を観測 |
...
| TC-E2E-001 | PASS | clean app-dataのrelease製品でroot登録→folder/ZIP/CBZ閲覧→読書位置/表示設定保存→main window正常close、10秒以内exit code 0→再起動しroot、保存page 3/3、見開き、左読みを復元 |
...
| TC-E2E-004 | BLOCKED | clean隔離VM、offline installer/WebView2、VM外DNS/TCP/UDP監視が必要。全MVP完走・通信0・原本差分0の手順と証跡条件をmanual procedureへ固定 |
