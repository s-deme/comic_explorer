...
use settings_commands::{
    apply_settings_profile_to_settings, decode_stored_settings_profile,
    normalize_settings_profile_input, settings_profile_changed_fields,
    settings_profile_confirmation_key, stored_settings_profile,
    validate_named_settings_profile_name, validate_settings_profile,
};
...
use crate::state::{
    AppPaths, BookmarkRecord, FavoriteRecord, NamedSettingsProfileRecord, Settings, StateStore,
    ThumbnailPins, ThumbnailPipeline,
};
...
pub struct AppState {
    ...
    file_undo: Arc<Mutex<file_operations::FileUndoJournal>>,
...
impl Default for AppState {
    fn default() -> Self {
        ...
        Self {
            ...
            file_undo: Arc::new(Mutex::new(file_operations::FileUndoJournal::default())),
...
fn read_page_bytes(
    ...
    let result = read_grant_bytes(grant).and_then(|bytes| {
        ...
        if !metadata.animated {
            if let Some(png) = crate::catalog::color_managed_png(&bytes)? {
                return Ok(("image/png", png));
            }
        }
        match metadata.format {
            ImageFormat::Bmp | ImageFormat::Tiff | ImageFormat::Ico | ImageFormat::Avif => Ok((
                "image/png",
    ...
    fn fr_b19_and_req_ley_p3_013_to_p3_015_and_p3_019_and_req_ley_p4_004_settings_profile_validates_all_atomic_bindings()
     {
        ...
        let (_, _, _, _, gestures) = validate_settings_profile(&profile).unwrap();
        ...
        profile.viewer_quadrant_bindings = default_viewer_quadrant_bindings();
        profile.viewer_right_click_action = "delete".into();
        assert_eq!(
            validate_settings_profile(&profile).unwrap_err().code,
            ErrorCode::InvalidRequest
        );
