...
| 種類 | 対応形式 |
|---|---|
| 画像 | BMP、JPEG/JPG、GIF、TIFF/TIF、PNG、ICO、SVG、WebP（アニメーションを含む）、AVIF（静止画・画像構造による制限あり） |
| 文書 | PDF |
...
ビューワ下部の「プレビュー」では、ページ番号を指定して画像とファイル名を確認できます。
「このページへ移動」を押すまで読書位置は変わりません。Escまたは「閉じる」で読書へ戻れます。
「全ページ一覧」で画像を並べて探せます。上部の「連続縦読み」で横幅に合わせたスクロール表示へ切り替えられ、現在ページは保存されます。倍率・回転操作を使うとページ送り表示へ戻ります。
...
- Windows専用です。
- AVIFは同梱デコーダーで表示・サムネイル・フィルター・コピーに対応します。静止画の主画像が対象で、グリッド合成・HDR・一部の画像構造は未対応です。
- JPEG・PNG・WebP・TIFF・AVIFの埋め込みRGB ICCはsRGBへ変換します。CMYK等の非RGBプロファイルは未対応です。モニターへの最終表示はWebView2の色管理を利用します。
- アニメーションWebPは1024フレーム以内かつキャンバスの作業メモリ上限内で再生します。サムネイル・フィルター・画像コピーでは先頭フレームを使います。
