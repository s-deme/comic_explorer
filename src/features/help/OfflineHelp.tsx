...
const HELP_TOPICS: readonly HelpTopic[] = [
  ...
  },
  {
    ...
    sections: [
      {
        title: "ページを移動する",
        body: "PageDownキーで次へ、PageUpキーで前へ移動します。「連続縦読み」では横幅に合わせてスクロールできます。下部の「プレビュー」→「全ページ一覧」で画像を並べて探し、「このページへ移動」で確定します。プレビューを閉じるだけでは読書位置は変わりません。作品を閉じて一覧へ戻るにはEscキーを押します。",
      },
      {
        title: "表示を切り替える",
        body: "単ページ・見開き、読み方向、表示倍率を切り替えられます。細部を確認するときはルーペやグリッド、読書位置を残すときはしおりを利用できます。",
  ...
  },
  {
    ...
    sections: [
      {
        title: "検索ペインを開く",
        body: "Ctrl+Fキーでフォルダーツリーと検索ペインを切り替えます。検索範囲を現在のフォルダー、ライブラリ全体、指定した複数の場所から選び、名前・結果の種類・サイズ・更新日を指定できます。",
      },
      {
        title: "検索結果から戻る",
